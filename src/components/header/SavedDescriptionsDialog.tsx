...
const SavedDescriptionsDialog: React.FC<SavedDescriptionsDialogProps> = ({
  ...
  const { loadSavedDescriptions, deleteSavedDescription } = useEditorStore();
  ...
  const handleDeleteConfirm = async () => {
    if (selectedDescription) {
      const success = await deleteSavedDescription(selectedDescription.id);
      ...
      if (success) {
        ...
        toast({
          variant: "destructive",
          title: "Erro",
          description: "Não foi possível excluir a descrição."
        });
      }
    }
