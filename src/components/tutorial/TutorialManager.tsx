...
const TutorialManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!hasVisitedBefore) {
      ...
      toast({
        ...
        action: <Button size="sm" onClick={() => setShowTutorial(true)}>
            Iniciar Tour
          </Button>
      });
    }
  ...
  return <>
      <Button variant="ghost" size="sm" onClick={() => setShowTutorial(true)} className="flex items-center text-xs">
        <HelpCircle className="h-4 w-4 mr-1" />
        <span className="hidden md:inline">Ajuda</span>
      </Button>
      
      <TutorialPopup open={showTutorial} onOpenChange={setShowTutorial} steps={tutorialSteps} />
    </>;
};
