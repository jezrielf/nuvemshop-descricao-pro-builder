 ...
 * with the style block restored to maintain consistent styling
 */
export const generateCompleteHtml = (state: EditorState): string => {
  ...
  // Style block that was requested to be restored
  const styleBlock = `
<style>
  /* Reset básico */
  ...
  for (let i = 0; i < visibleBlocks.length; i++) {
    ...
    try {
      ...
      blocksHtml += `<div style="${marginClass}" id="product-block-${block.id}" class="product-description-block">\n`;
  ...
  // Return blocks HTML with style block but without the problematic outer div container
