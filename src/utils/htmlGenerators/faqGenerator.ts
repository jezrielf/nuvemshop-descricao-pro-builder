...
export const generateFAQHtml = (block: FAQBlock): string => {
  ...
  // Generate each FAQ item with a more minimalist, cleaner design
  const faqHtml = block.questions && block.questions.length > 0 
    ? block.questions.map((item, index) => `
      <div class="faq-item" style="margin-bottom:10px;border-bottom:1px solid #eaeaea;overflow:hidden;">
  ...
  // CSS específico para este acordeão
  const faqCss = `
    <style>
      ...
      #faq-block-${block.id} .faq-question:hover {
        color: #000;
      }
      #faq-block-${block.id} .faq-item:last-child {
        margin-bottom: 0;
        border-bottom: none;
      }
    </style>
  ...
  // HTML completo com todas as tags devidamente fechadas
  return `
