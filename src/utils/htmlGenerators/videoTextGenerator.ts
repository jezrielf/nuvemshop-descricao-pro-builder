...
import { VideoTextBlock } from "@/types/editor/blocks/videoText";
...
export function generateVideoTextBlockHtml(block: VideoTextBlock): string {
  ...
  const videoEmbed = embedUrl 
    ? `<div class="video-text-embed" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden;">
         <iframe 
           src="${embedUrl}" 
  ...
  return `
    <div class="video-text-block ns-row" style="${containerStyle}">
