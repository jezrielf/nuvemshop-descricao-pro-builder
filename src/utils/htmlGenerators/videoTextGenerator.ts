...
export function generateVideoTextBlockHtml(block: VideoTextBlock): string {
  try {
    ...
    return `
      <div class="video-text-block my-6" style="${containerStyle}">
    ...
    console.error('Error generating VideoText block HTML:', error);
    return `
      <div class="error-block p-4 bg-red-50 text-red-600 rounded">
        Erro ao gerar bloco de Vídeo e Texto: ${error instanceof Error ? error.message : 'Erro desconhecido'}
      </div>
    `;
  }
}
